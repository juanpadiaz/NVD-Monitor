...
install_dependencies() {
    ...
    if dpkg -l 2>/dev/null | grep -q "^ii.*mariadb-server\|^ii.*mysql-server"; then
        db_exists=true
        log_info "Base de datos detectada, preservando instalación"
    fi
...
#!/usr/bin/env python3
"""NVD Vulnerability Monitor v1.0.5"""
...
def main():
    parser = argparse.ArgumentParser(description='NVD Monitor v1.0.5')
...
configure_email() {
    ...
    if [[ $configure_mail =~ ^[Yy]$ ]]; then
        ...
        while true; do
            read -p "Email destinatario (alertas): " RECIPIENT_EMAIL
...
finalize_installation() {
    ...
    if ! sudo -u nvd-monitor cat "$CONFIG_DIR/config.ini" >/dev/null 2>&1; then
        log_error "Error de permisos en configuración"
        exit 1
    fi
    ...
    if systemctl is-active --quiet nvd-monitor; then
        ...
        echo "Ver logs: sudo journalctl -u nvd-monitor -n 20"
        exit 1
    fi
}
...
show_summary() {
    ...
    echo
    echo "🔧 COMANDOS:"
    echo "• nvd-status           - Ver estado"
    echo "• nvd-admin test-db    - Probar BD"
    echo "• nvd-admin status     - Estado detallado"
    echo
    echo "📁 ARCHIVOS:"
    echo "• Config: /etc/nvd-monitor/config.ini"
    echo "• Logs: /var/log/nvd-monitor/"
    echo
    
    if [[ -z "$API_KEY" ]]; then
        echo "💡 Para mejor rendimiento, configure API key:"
        echo "   • Obtenga en: https://nvd.nist.gov/developers/request-an-api-key"
