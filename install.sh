...
setup_mysql() {
    ...
    log_success "MySQL/MariaDB está ejecutándose"
}
...
setup_database() {
    ...
    # Intentar conectar sin contraseña root primero
    if mysql -u root -e "SELECT 1;" &>/dev/null; then
        log_info "Creando base de datos y usuario..."
    else
        log_warn "MySQL requiere contraseña root"
        echo "Ingrese la contraseña root de MySQL:"
        read -s mysql_root_password
        if ! mysql -u root -p"$mysql_root_password" -e "SELECT 1;" &>/dev/null; then
            log_error "Contraseña root incorrecta"
            exit 1
        fi
