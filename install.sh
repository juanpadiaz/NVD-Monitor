...
def main():
    parser = argparse.ArgumentParser(description='NVD Vulnerability Monitor v1.0.3')
    ...
    parser.add_argument('--version', action='version', version='NVD Monitor 1.0.3')
...
# Crear script de configuración de base de datos simplificado
create_database_setup_script() {
    cat > /tmp/setup_database_simple.sh << 'EOF'
#!/bin/bash
...
log_info "Configurando base de datos para NVD Monitor..."

# Verificar que MariaDB o MySQL esté ejecutándose
if systemctl is-active --quiet mariadb; then
    DB_SERVICE="mariadb"
...
log_info "Detectado: $DB_SERVICE"

# Generar contraseña
nvd_password=$(generate_password)
...
if [ $? -eq 0 ]; then
    log_info "Base de datos y tablas creadas correctamente"
else
...
log_info "✅ Configuración de base de datos completada"

# Verificar conexión
if command -v nvd-admin &>/dev/null; then
    if nvd-admin test-db &>/dev/null; then
        log_info "✅ Verificación de conexión exitosa"
    else
        log_info "⚠️  Verificar configuración manualmente"
    fi
fi
EOF

    chmod +x /tmp/setup_database_simple.sh
}
...
main_installation() {
    ...
    # Configurar logrotate y utilidades
    setup_logrotate
    create_utility_scripts
    set_final_permissions
...
show_final_summary() {
    ...
    if [[ $REPLY =~ ^[Nn]$ ]]; then
        ...
        echo "Configure manualmente la base de datos más tarde."
    else
        ...
        if bash /tmp/setup_database_simple.sh; then
            log_success "Base de datos configurada correctamente"
        else
            log_error "Error configurando base de datos"
            echo "Configure manualmente siguiendo la documentación"
        fi
        
        # Limpiar script temporal
        rm -f /tmp/setup_database_simple.sh
    fi
...
show_help() {
    ...
    echo ""
    echo "Este script instalará NVD Monitor en Ubuntu ${SUPPORTED_UBUNTU} LTS y superiores"
