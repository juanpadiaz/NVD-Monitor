...
# Instalar dependencias con detección mejorada
install_dependencies() {
    log_step "Instalando dependencias..."
    
    # Detectar base de datos existente con múltiples métodos
    local db_exists=false
    local db_type=""
    
    # Método 1: Verificar paquetes instalados
    ...
    # Método 2: Verificar servicios systemd
    ...
    # Método 3: Verificar comando mysql
    ...
    if [ "$db_exists" = true ]; then
        log_info "$db_type ya está instalado - usando instalación existente"
    else
        log_info "No se detectó base de datos - instalando MariaDB"
    fi
    ...
    # Solo instalar MariaDB si no existe
    ...
    # Configurar MariaDB solo si se instaló nuevo
    if [ "$db_exists" = false ]; then
        systemctl enable mariadb
        systemctl start mariadb
        log_info "MariaDB instalado y configurado"
    else
        # Asegurar que esté ejecutándose
        ...
        log_info "$db_type verificado y ejecutándose"
    fi
...
# Configurar base de datos con manejo robusto de errores
setup_database() {
    ...
    # Probar diferentes métodos de autenticación para MariaDB/MySQL
    local mysql_cmd=""
    local auth_success=false
    
    # Método 1: Sin contraseña (común en instalaciones nuevas)
    ...
    # Método 2: Con sudo (común en MariaDB)
    if [ "$auth_success" = false ] && sudo mysql -u root -e "SELECT 1;" &>/dev/null 2>&1; then
        mysql_cmd="sudo mysql -u root"
        auth_success=true
        log_info "Usando autenticación con sudo"
    fi
    
    if [ "$auth_success" = false ]; then
        log_error "No se pudo autenticar con MySQL/MariaDB"
        log_error "Posibles soluciones:"
        log_error "  • Ejecutar: sudo mysql_secure_installation"
        log_error "  • Verificar que MariaDB esté ejecutándose"
        exit 1
    fi
...
EOSQL
    ...
    # Ahora necesitamos ejecutar las queries SQL con la contraseña real
    # ya que el heredoc anterior no expande variables cuando está entre comillas simples
    $mysql_cmd -e "DROP USER IF EXISTS 'nvd_user'@'localhost';"
    $mysql_cmd -e "CREATE USER 'nvd_user'@'localhost' IDENTIFIED BY '${DB_PASSWORD}';"
    $mysql_cmd -e "GRANT ALL PRIVILEGES ON nvd_monitor.* TO 'nvd_user'@'localhost';"
...
# Configuración de email con soporte para múltiples destinatarios
configure_email() {
    ...
    if [[ $configure_mail =~ ^[Yy]$ ]]; then
        ...
        # Emails destinatarios (múltiples)
        echo
...
EOF
    ...
    # Permisos correctos para que nvd-monitor pueda leer
...
# Finalizar instalación con verificaciones mejoradas
finalize_installation() {
    log_step "Finalizando instalación..."
    
    # Crear grupo nvd-monitor si no existe
    if ! getent group nvd-monitor >/dev/null 2>&1; then
        groupadd nvd-monitor
        usermod -a -G nvd-monitor nvd-monitor
    fi
    
    # Verificar que nvd-monitor puede leer la configuración
    if ! sudo -u nvd-monitor cat "$CONFIG_DIR/config.ini" >/dev/null 2>&1; then
        log_error "Error: Usuario nvd-monitor no puede leer la configuración"
        ...
        if ! sudo -u nvd-monitor cat "$CONFIG_DIR/config.ini" >/dev/null 2>&1; then
            log_error "No se pudieron corregir los permisos"
            exit 1
        fi
    fi
    
    log_success "Permisos de configuración verificados"
    ...
    if nvd-admin test-db; then
        ...
        echo "🔍 Diagnóstico:"
        echo "• Verificando usuario en MySQL:"
        mysql -u root -e "SELECT User, Host FROM mysql.user WHERE User='nvd_user';" 2>/dev/null || echo "  Error consultando usuarios"
        
        echo "• Verificando base de datos:"
        mysql -u root -e "SHOW DATABASES;" 2>/dev/null | grep nvd_monitor || echo "  Base de datos nvd_monitor no encontrada"
...
show_summary() {
    ...
    echo "• nvd-admin status     - Estado detallado"
    ...
    echo "• Usuario: nvd_user"
    echo "• Host: localhost:3306"
    
    # Mostrar estadísticas de la BD
    echo
    echo "📊 ESTADÍSTICAS:"
    if mysql -u nvd_user -p"$DB_PASSWORD" nvd_monitor -e "
    SELECT 
        'Tablas creadas' as 'Elemento',
        COUNT(*) as 'Cantidad'
    FROM information_schema.tables 
    WHERE table_schema = 'nvd_monitor'
    UNION ALL
    SELECT 
        'Configuraciones',
        COUNT(*) 
    FROM system_config;" 2>/dev/null; then
        echo "✅ Base de datos operativa"
    else
        echo "⚠️  Error consultando estadísticas de BD"
    fi
    
    echo
    if [[ -z "$API_KEY" ]]; then
        echo "💡 RECOMENDACIONES:"
        echo "1. 🔑 Configurar API Key de NVD para mejor rendimiento:"
    ...
    if [[ -z "$SENDER_EMAIL" ]]; then
        echo "2. 📧 Configurar notificaciones por email:"
        echo "   • Edite: sudo nano /etc/nvd-monitor/config.ini"
    ...
    echo "• Ver logs en tiempo real: sudo journalctl -u nvd-monitor -f"
    echo "• Para soporte: verificar documentación en GitHub"
...
main() {
    case "${1:-}" in
        -h|--help)
            ...
            echo "• Detección inteligente de MariaDB/MySQL existente"
            echo "• Configuración interactiva de API Key y email"
            echo "• Soporte para múltiples destinatarios de email"
            echo "• Verificaciones robustas de funcionamiento"
            echo "• Manejo mejorado de errores y diagnósticos"
            echo
...
error_handler() {
    ...
    case $exit_code in
        1)
            echo
            echo "🛠️  POSIBLES CAUSAS Y SOLUCIONES:"
            echo "• Error de permisos: Asegurar ejecución como root (sudo)"
            echo "• Error de conectividad: Verificar conexión a internet"
            echo "• Error de base de datos: Verificar MariaDB/MySQL"
            ;;
        127)
            echo
            echo "🛠️  COMANDO NO ENCONTRADO:"
            echo "• Instalar dependencias faltantes"
            echo "• Verificar PATH del sistema"
            ;;
        *)
            echo
            echo "🛠️  SOLUCIONES GENERALES:"
            echo "• Verificar espacio en disco disponible"
            echo "• Revisar logs del sistema: sudo journalctl -n 50"
            echo "• Reintentar la instalación"
            ;;
    esac
    
    echo
    echo "📧 Para soporte técnico, incluya esta información completa"
